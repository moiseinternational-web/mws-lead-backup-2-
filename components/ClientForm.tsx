...
import type { Client, Service, LeadField, LeadFieldType, User } from '../types';
...
const ClientForm: React.FC<ClientFormProps> = ({ client, onSuccess }) => {
        ...
        if (!isEditing) {
            const fetchUsers = async () => {
                ...
                try {
                    ...
                    if (users.length > 0) {
                        setUserId(users[0].id); // Pre-select the first user
                    }
                } catch (e) {
                    setError("Impossibile caricare gli utenti disponibili.");
                } finally {
    ...
    useEffect(() => {
        if (isEditing && client) {
            setName(client.name);
            setUserId(''); // Not needed for editing
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isEditing && client) {
                ...
                if (!userId) {
                    throw new Error("Seleziona un utente da associare al cliente.");
                }
                await ApiService.addClient(name, userId, finalServices as Omit<Service, 'id'>[], quoteWebhookUrl);
            }
            onSuccess();
        } catch (err: any) {
    ...
    return (
        <>
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {!isEditing && (
                    <div>
                        <label htmlFor="user-select" className="block text-sm font-medium text-slate-700 dark:text-gray-300">Utente da Associare</label>
                        {isLoadingUsers ? (
                            <div className={`${inputClasses} mt-1 text-slate-500`}>Caricamento utenti...</div>
                            ...
                            <div className="mt-1 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-500/30 rounded-md text-sm text-yellow-700 dark:text-yellow-300">
                                Nessun nuovo utente disponibile da associare. Chiedi al nuovo cliente di registrarsi prima.
                            </div>
                        )}
                    </div>
                )}
